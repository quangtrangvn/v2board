...
class V2boardInstall extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $this->info("   ");
			$this->info(" => Việt hóa bởi NK Quang - Mua4g.com ");
            if (\File::exists(base_path() . '/.env')) {
                $securePath = config('v2board.secure_path', config('v2board.frontend_admin_path', hash('crc32b', config('app.key'))));
                $this->info("vào http(s)://domain_của_bạn/{$securePath} Bạn có thể sửa link này ở trang admin");
            ...
            $this->saveToEnv([
                ...
                'DB_USERNAME' => $this->ask('Nhập tên user database'),
            ...
            $email = '';
            while (!$email) {
                $email = $this->ask('Nhập địa chỉ email của Admin?');
            }
            $password = Helper::guid(false);
            if (!$this->registerAdmin($email, $password)) {
                abort(500, 'Đăng ký tài khoản Admin không thành công, vui lòng thử lại');
            }
            ...
            $this->info("Truy cập http(s)://domain_của_bạn/{$defaultSecurePath} Vào trang Admin, bạn có thể sửa đổi link của mình.");
        } catch (\Exception $e) {
